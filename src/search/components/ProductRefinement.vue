<template>
  ...
  <v-card v-if="searchPage?.breadcrumbs?.length">
    ...
    <v-list>
      <v-list-item v-for="breadcrumb in searchPage?.breadcrumbs"
            :key="breadcrumb.facetValueCode"
            :title="breadcrumb.facetValueName"
            :link="true" :href="routerPath(breadcrumb.removeQuery?.url)"
    ...
    <div v-if="facetVisible(facet)">
      <v-list v-if="facet.multiSelect">
        ...
        <v-list-item v-for="facetValue in facet.values"
            ...
            :link="true" :href="routerPath(facetValue.query?.url)"
